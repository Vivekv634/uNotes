...
const UpdateNote = () => {
    ...
    const fetchNote = async () => {
        const response = await axios.get(`http://localhost:5500/user/notes/${noteID}?id=${Cookies.get('userTokenID')}`);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const response = await axios.put(`http://localhost:5500/user/notes/update/${noteID}?id=${Cookies.get('userTokenID')}`, { title, body });
        const result = await response.data;
        if (result.error) {
            alert(result.error);
        } else {
